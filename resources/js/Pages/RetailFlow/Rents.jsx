...
export default function RentsRetailFlow() {
    ...
    const [statpaid, setStatpaid] = useState(false);
...
const openEdit = (rent) => {
    ...
    setForm({
        ...rent,
        start_date: rent.start_date ? rent.start_date.split('T')[0] : '',
        end_date: rent.end_date ? rent.end_date.split('T')[0] : '',
    });
    setEditModal(true);
};
    ...
    const handelpaid = async () => {
        try {
            await axios.post(
                `${app_url}/rentsretailFlow/paid/${selectedRent.id}`,
                {
                    paid_amount:paid
                }
            );
    ...
    const handleSaveEdit = async () => {
        try {
            await axios.post(
                `${app_url}/rentsretailFlow/${selectedRent.id}`,
                form,
                {
                    headers: { "Content-Type": "application/json" },
                }
            ...
            <div className="mx-3 bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-6 mb-10">
                {auth.user.company.subscription === "vip" && (
                    <div className="flex gap-2 mb-4">
                        <button
                            onClick={() =>
                                (window.location.href = `${app_url}/retailflow/export/rents/excel`)
                            }
                        ...
                        <button
                            onClick={() =>
                                (window.location.href = `${app_url}/retailflow/export/rents/pdf`)
                            }
                        ...
                        <div className="flex items-center ">
                            ...
                            <input
                                ...
                                placeholder={
                                    isGym
                                        ? t(
                                              "ابحث باسم العميل (المشترك) أو رقم الاشتراك..."
                                          )
                                        : t(
                                              "ابحث باسم العميل (المستأجر) أو رقم العقد..."
                                          )
                                }
                ...
                <div className="overflow-x-auto">
                    <table className="min-w-full table-fixed">
                        ...
                        <thead className="bg-gray-50 dark:bg-gray-700 sticky top-0 z-10">
                            <tr>
                                ...
                                <th className="px-4 py-3 text-right text-xs font-bold text-gray-600 dark:text-gray-300 uppercase tracking-wider">
                                    {isGym
                                        ? t("قيمة الاشتراك الشهري")
                                        : t("قيمة الإيجار الشهري")}
                                </th>
                                <th className="px-4 py-3 text-right text-xs font-bold text-gray-600 dark:text-gray-300 uppercase tracking-wider">
                                    {t("تاريخ البداية ")}
                                </th>
                        ...
                        <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
                            {currentRents.map((rent, idx) => (
                                ...
                                >
                                    ...
                                    <td className="px-4 py-3 text-right text-gray-700 dark:text-gray-200">
                                        {Number(rent.monthly_rent || 0).toFixed(
                                            2
                                        )}
                                    </td>
                                    <td className="px-4 py-3 text-right text-gray-700 dark:text-gray-200">
                                    ...
                                    <td className="px-4 py-3 text-right text-gray-700 dark:text-gray-200">
                                        {formatDate(rent.next_rent_date)}
                                    </td>
                                    <td className="px-4 py-3 text-right text-gray-700 dark:text-gray-200">
                                        {Number(rent.paid_amount || 0).toFixed(
                                            2
                                        )}
                                    </td>
                                    <td className="px-4 py-3 text-right text-gray-700 dark:text-gray-200">
                                        {rent.subscription_type === "monthly"
                                            ? t("شهري")
                                            : rent.subscription_type ===
                                              "yearly"
                                            ? t("سنوي")
                                            : "-"}
                                    </td>
                    ...
                    {filteredRents.length === 0 && (
                        <div className="text-center py-8 text-gray-500 dark:text-gray-400">
                            {t("لا توجد")} {isGym ? t("اشتراكات") : t("عقود")}{" "}
                            {t("لعرضها")}
                        </div>
                    )}
                    ...
                    {filteredRents.length > rowsPerPage && (
                        <div className="flex justify-between items-center mt-4">
                            ...
                            <span className="text-gray-700 dark:text-gray-300">
                                ...
                                {Math.ceil(filteredRents.length / rowsPerPage)}
                            </span>
                            <button
                                onClick={() => setCurrentPage(currentPage + 1)}
                                disabled={
                ...
                {editModal && selectedRent && (
                    <RentModal
                        title={
                            isGym
                                ? `${t("تعديل الاشتراك")} #${selectedRent.id}`
                                : `${t("تعديل العقد")} #${selectedRent.id}`
                        }
                        customers={customers}
                        form={form}
                        setForm={setForm}
                ...
                {statpaid && <PaidModel onClose={closeModals} onSave={handelpaid} paid={paid} setPaid={setPaid}  selectedRent={selectedRent} t={t} errors={errors}/>}
                        ...
                        <div className="bg-white dark:bg-background-card rounded-xl shadow-2xl max-w-md w-full">
                            <div className="flex items-center justify-between p-6 border-b border-gray-200 dark:border-gray-700">
                                <h3 className="text-lg font-bold text-gray-800 dark:text-gray-200">
                                    {isGym
                                        ? `${t(
                                              "هل أنت متأكد من حذف الاشتراك"
                                          )} #${selectedRent.id}؟`
                                        : `${t("هل أنت متأكد من حذف العقد")} #${
                                              selectedRent.id
                                          }؟`}
                                </h3>
                                <button
                            ...
                            <div className="p-6">
                                <p className="text-gray-600 dark:text-gray-400 mb-4">
                                    {isGym
                                        ? t(
                                              "سيتم حذف هذا الاشتراك بشكل دائم ولا يمكن التراجع عن هذه العملية."
                                          )
                                        : t(
                                              "سيتم حذف هذا العقد بشكل دائم ولا يمكن التراجع عن هذه العملية."
                                          )}
                                </p>
                            ...
                            <div className="p-6 print:p-8">
                                ...
                                <div className="space-y-4 mb-6">
                                    ...
                                    <div className="flex justify-between">
                                        ...
                                        <span>
                                            {Number(
                                                (selectedRent.monthly_rent ||
                                                    0) -
                                                    (selectedRent.paid_amount ||
                                                        0)
                                            ).toFixed(2)}{" "}
                                            {t("ج.م")}
                                        </span>
                                    ...
                                    <div className="flex justify-between">
                                        ...
                                        <span>
                                            {selectedRent.subscription_type ===
                                            "monthly"
                                                ? t("شهري")
                                                : selectedRent.subscription_type ===
                                                  "yearly"
                                    ...
                                    <div className="flex justify-between">
                                        ...
                                        <span>
                                            {formatDate(selectedRent.end_date)}
                                        </span>
                                    </div>
                                </div>
                                        ...
                                        <p className="text-gray-500 text-xs">
                                            {(auth.user.company.subscription ===
                                                ...
                                                auth.user.company
                                                    ...
                                                    "premium") &&
                                                t(
                                                    "سيستمى نظام متكامل لادارة الانشطة التجارية"
                                                )}
                                        </p>
                                    </div>
                                </div>
...
function RentModal({
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
            <div
                className="bg-white dark:bg-background-card rounded-xl shadow-2xl max-w-2xl w-full"
                onMouseLeave={() => setShowCustomerList(false)}
            >
                <div className="flex items-center justify-between p-6 border-b border-gray-200 dark:border-gray-700">
                    <h3 className="text-lg font-bold text-gray-800 dark:text-gray-200">
                        {title}
...
function PaidModel({  paid,setPaid, onClose, onSave,t,errors }) {
    return (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
            <div
                className="bg-white dark:bg-background-card rounded-xl shadow-2xl max-w-2xl w-full"
